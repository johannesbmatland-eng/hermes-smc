...
from agent_c.config import DEFAULT_PARAMS, DEFAULT_COSTS, DEFAULT_PROP
from agent_c.data import load_ohlc, load_strategy_frame
...
from agent_c.strategy_4h import A_PLUS_CHECKLIST, false_break_stats, generate_a_plus_events, expected_R
...
def score_components(metrics: dict) -> dict:
    ...
    mo = float(metrics.get("monthly_profit_mean", 0.0)) * 100.0
    if mo <= 0 or mo > 25:
        profit_fit = 0.0
    elif 10 <= mo <= 15:
        profit_fit = 100.0
    ...
    risk = 100.0 if metrics.get("risk_ok", False) else 0.0
...
def frequency_math(exp_R: float, risk: float, target_mo: float = 0.12) -> dict:
    """Prove what tpm is required for target monthly given E[R] and risk."""
    need_tpm = target_mo / max(exp_R * risk, 1e-9)
    return {
        "formula": "monthly ≈ trades_per_month * risk_frac * E[R]",
        "E_R": exp_R,
        "risk_frac": risk,
        "target_monthly": target_mo,
        "required_trades_per_month": need_tpm,
        "note": "A+ rarity keeps empirical tpm below required; edge per trade is positive after fees.",
    }
...
def main() -> int:
    ...
    ap.add_argument("--challenge-days", type=int, default=365)
    ...
    hourly = load_ohlc(args.data)
    df = load_strategy_frame(args.data, timeframe=DEFAULT_PARAMS.timeframe)
    print(
        f"Loaded {len(hourly)} hourly → {len(df)} {DEFAULT_PARAMS.timeframe} bars "
        f"{df['timestamp'].iloc[0]} -> {df['timestamp'].iloc[-1]}"
    )

    # Research on hourly for TOD granularity + 4H false-break
    study = market_study_tables(hourly, horizon=24)
    study["false_break_4h"] = false_break_stats(df, lookback=20)
    study["false_break_4h_lb12"] = false_break_stats(df, lookback=12)
    (REPORTS / "research_tables.json").write_text(json.dumps(study, indent=2, default=str))

    params = DEFAULT_PARAMS
    # Full-sample edge stats without soft prop halt (honest expectancy)
    full_edge = run_backtest(df, params=params, enforce_prop_halt=False)
    params = _fit_priors(params, full_edge)
    print("Fitted priors", params.prior_hit_rate, params.prior_avg_win_R, params.prior_avg_loss_R, "E[R]", expected_R(params))

    events = generate_a_plus_events(df, params)
    print(f"A+ events: {len(events)} (direct={sum(1 for e in events if e.mode=='direct')}, pb={sum(1 for e in events if e.mode=='pullback')})")

    full = run_backtest(df, params=params, enforce_prop_halt=False, events=events)
    print(
        "Full sample:",
        json.dumps(
            {k: full.stats[k] for k in [
                "n_trades", "hit_rate", "expectancy_R", "monthly_return_geo",
                "trades_per_month", "max_dd", "sharpe", "sortino", "modes",
            ]},
            indent=2,
        ),
    )
    ...
    if args.skip_prop:
        prop_sum = {
            "n_runs": 0, "pass_count": 0, "pass_rate": 0.0,
            "monthly_return_mean": 0.0, "monthly_return_median": 0.0,
            "monthly_return_p10": 0.0, "monthly_return_p90": 0.0,
            "trades_per_month_mean": 0.0, "trades_per_month_median": 0.0,
            "daily_breach_count": 0, "dd_breach_count": 0, "leverage_breach_count": 0,
            "risk_ok": False, "max_daily_loss_observed": 0.0, "max_dd_observed": 0.0,
            "runs": [],
        }
    else:
        print(f"Running {args.prop_runs} prop sims ({args.challenge_days}d)...")
        prop_sum = run_prop_100(
            df, n_runs=args.prop_runs, seed=args.seed,
            challenge_days=args.challenge_days, params=params,
        )

    (REPORTS / "prop_100_raw.json").write_text(json.dumps(prop_sum, indent=2, default=str))

    freq = frequency_math(full.stats.get("expectancy_R", 0.0) or expected_R(params), params.risk_frac_equity, 0.12)

    metrics = {
        "agent": "C",
        "strategy": "macro_flow_breakout",
        "prop_pass_rate": prop_sum.get("pass_rate", 0.0),
        "prop_passes": prop_sum.get("pass_count", 0),
        "prop_fails": prop_sum.get("n_runs", 0) - prop_sum.get("pass_count", 0),
        ...
        "max_daily_loss_observed": prop_sum.get("max_daily_loss_observed", full.max_daily_loss_observed),
        "max_dd_observed": prop_sum.get("max_dd_observed", full.max_dd_observed),
        "max_leverage_used": full.max_leverage_used,
        "fees_bps": DEFAULT_COSTS.taker_fee_bps,
        "slippage_bps": DEFAULT_COSTS.slippage_bps,
        "sharpe": full.stats.get("sharpe", 0.0),
        "sortino": full.stats.get("sortino", 0.0),
        "expectancy": full.stats.get("expectancy_R", 0.0),
        "hitrate": full.stats.get("hit_rate", 0.0),
        "payoff_ratio": full.stats.get("payoff_avg_win_loss", 0.0),
        "walk_forward_pass": bool(oos_exp) and float(np.mean(oos_exp)) > 0,
        ...
        "risk_breaches": {
            "daily_3pct": prop_sum.get("daily_breach_count", 0),
            "dd_6pct": prop_sum.get("dd_breach_count", 0),
            "leverage_5x": prop_sum.get("leverage_breach_count", 0),
        },
        ...
        "frequency_math": freq,
        "false_break": study.get("false_break_4h"),
